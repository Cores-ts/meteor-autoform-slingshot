afSlingshot.Model = function(config, data, multi) {
    ...
    if (data) {
        _.each(_.uniq(_.keys(data)),function(filename) {
            ...
            console.log(self._uploads[ filename ]);
        });
    }
};
...
afSlingshot.Model.prototype.error = function(name) {
    console.log('model' + this._uploads[ name ].error());
    return this._uploads[ name ].error();
};
