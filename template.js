...
SimpleSchema.messages({ wrongFilename: 'Filename not valid' });

AutoForm.addInputType('slingshot', {
    template: 'afSlingshot',
    valueOut: function() {
        var afSlingshot = hackySlingshotStorage[this.attr('data-schema-key')];
        //var validationContext = AutoForm.getValidationContext('blockUpdate');
        ...
        if (afSlingshot.progress() < 100) {
            ...
            AutoForm.addStickyValidationError('blockUpdate', this.attr('data-schema-key'), "uploadInProgress", val)
        } else if ((typeof val == "undefined" || val == "") && this[0].required) {
            /*validationContext.addInvalidKeys([{
                name: this.attr('data-schema-key'),
                type: 'required'
            }]);*/
            AutoForm.addStickyValidationError('blockUpdate', this.attr('data-schema-key'), "required", val)
        } else if ((typeof val != "undefined" || val != "") && (val && val !=  "undefined" && val.src && val.src.startsWith("blob:"))) {
            /*validationContext.addInvalidKeys([{
                name: this.attr('data-schema-key'),
                type: 'wrongFilename',
                value: val
            }]);*/
            AutoForm.addStickyValidationError('blockUpdate', this.attr('data-schema-key'), "wrongFilename", val)
        } else {
            AutoForm.removeStickyValidationError('blockUpdate', this.attr('data-schema-key'))
            return val;
        }
    },
    valueConverters: {
...
Template.afSlingshot.helpers({
    ...
    summary: function() {
        ...
        if (files.length === 0) {
            ...
            return files[0].filename.substring(files[0].filename.lastIndexOf('/') + 1);
        } else {
    ...
    docName: function(url) { // from file scope
        if (url) {
